...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def on_pushButton_start_combination_clicked(self):
        ...
        if  len(strategy_list) > 3:
            print('最多選擇3種策略')
            QMessageBox.information(self, "注意", "最多選擇3種策略")
            return None

        if  len(strategy_list) == 0:
            print('最少選擇1種策略')
            QMessageBox.information(self, "注意", "最少選擇1種策略")
            return None
        ...
        w = [0.4, 0.2, 0.4]
        df['組合'] = (df * w).sum(axis=1)


        df_hs300 = pd.read_excel(r'data\組合.xlsx',index_col=[0], sheetname='Sheet2')
