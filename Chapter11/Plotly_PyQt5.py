...
class Plotly_PyQt5():
    ...
    def get_plotly_path_period_return(self, file_name='period_return.html'):
        ...
        滬深300 = [1.64,1.64,0.73,5.01,14.20]
        同類平均 = [-0.79,-0.79,-2.08,0.76,7.03]
        trace1 = go.Bar(
            x=['今年以來', '最近一個月', '最近三個月', '最近半年', '最近一年'],
            y=本基金,
            name='本基金'
        )
        trace2 = go.Bar(
    ...
    def get_plotly_path_product_vs_hs300(self, file_name='product_vs_hs300.html'):
        ...
        data = pd.read_excel(r'data\和聚光明1號_hs300_merge.xlsx', index_col=[0]
                             )
    ...
    def get_plotly_path_monte_markovitz(self, file_name='monte_markovitz.html',monte_count=400,risk_free = 0.03):
        ...
        """
        path_plotly = self.path_dir_plotly_html + os.sep + file_name
        df = pd.read_excel(r'data\組合.xlsx',index_col=[0])
        ...
        for p in range(monte_count):
            ...
            port_returns.append(np.sum(returns.mean() * 50 * weights)) # 加入模擬的平均值
            port_variance.append(np.sqrt(np.dot(weights.T, np.dot(returns.cov() * 50, weights)))) # 加入模擬的標準差

        port_returns = np.array(port_returns)
        port_variance = np.array(port_variance)
        color_array = (port_returns - risk_free) / port_variance # sharp比，不同的sharp比對應至不同的顏色
        ...
        trace1 = go.Scatter(
            ...
            marker=dict(
                size='6',
                color=color_array,  # 透過一個可變的變數表示顏色，結果是繪圖顏色可變
                colorscale='Viridis',
                # 設定colorbar
                colorbar=dict(
                    tickmode='linear',
    ...
    def get_plotly_path_combination_table(self,file_name='產品組合資訊表.html',df=None,w=None):
        '''畫產品組合資訊表
        ...
        _temp = {i[0]: i[1] for i in zip(['風險', '均值', 'sharp比', '最大回撤'], [std_year, mean_year, sharp_year, max_drawback])}
    ...
    def get_plotly_path_combination_pie(self,file_name='產品組合成分圓餅圖.html',df=None,w=None):
        '''
        畫產品組合成分圓餅圖
        df為dataframe，末尾一行為組合。
        w為權重
    ...
    def get_plotly_path_combination_versus(self,file_name='產品組合VS滬深300.html',df=None,df_base=None,w=None):
        '''
        畫產品組合VS滬深300
        df為dataframe，末尾一列為組合。
        w為權重
        :return:
        '''
        path_plotly = self.path_dir_plotly_html + os.sep + file_name

        df_contra = pd.concat([df_base.close, df['組合']], axis=1, join='inner')
        df_contra = df_contra / df_contra.iloc[0, :]
        df_contra.rename(columns={'close': '滬深300'}, inplace=True)

        trace1 = go.Scatter(x=df_contra.index, y=df_contra.iloc[:, 0], mode='lines+markers',
                            name='滬深300', marker=dict(color='blue'))
        trace2 = go.Scatter(x=df_contra.index, y=df_contra.iloc[:, 1], mode='lines+markers',
                            name='產品組合', marker=dict(color='red'))
        data = [trace1, trace2]
        layout = {'title': '產品組合VS滬深300'}
        fig = dict(data=data, layout=layout)
